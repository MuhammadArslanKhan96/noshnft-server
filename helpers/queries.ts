...
export const collectionQueries = {
  ...
  getAllcollection:
    "SELECT collection.id AS collection_id, collection.name AS collection_name, collection.description AS collection_description, users.id AS user_id,users.name AS user_name,users.email AS user_email,users.image_url AS user_image_url, COALESCE(json_agg(json_build_object('nft_name', nfts.name,'nft_description', nfts.description, 'nft_price', nfts.price, 'nft_image_url', nfts.image_url)) FILTER (WHERE nfts.id IS NOT NULL), '[]') AS nfts FROM collection JOIN users ON collection.primary_owner = users.id LEFT JOIN nft_collection ON nft_collection.collection_id = collection.id LEFT JOIN nfts ON nfts.id = nft_collection.nft_id GROUP BY collection.id, users.id",
};
